...
/**
 * A provider service to download and extract the open orders information.
 * The response will be send via messaging bus
*/
class OpenOrders implements ProviderServiceInterface {
    ...
    public function process($message) : void
    {
        ...
        if (!empty($contents))
        {
            $data_dict = json_decode($contents, true);
             ...
             $data = [];
            if (array_key_exists('straightBets', $data_dict)) {
                $data = array();
                $ctr = 1;
                foreach ($data_dict["straightBets"] as $row)
                {
                    ...
                    $bet = array("provider" => "pin", "sport" => $sport,
                        "id" => $ctr, "username" => $username, "status" => $status, "odds" => (string) $row["handicap"], "score" => $score, "stake" => (string) $row["risk"], "to_win" => (string) $row["win"], "bet_id" => (string) $row["betId"], "reason" => $reason);
            ...
            $producer->send('OPEN-ORDERS', $message);
            OutPut::normal("OPENORDERS sent: " . $message . PHP_EOL);
        }
        else
        {
            throw new \Exception('PIN-OPENORDERS: Response content is empty');
        }
    }
    ...
    {
        $response = $this->request->get("/v3/bets?betlist=RUNNING&fromDate={$fromDate}&toDate={$toDate}");
     ...
     *
     */
    private function getStatus($row) : string
    {
        if ($row["betStatus"] == "ACCEPTED")
        {
            return "success";
        }
        elseif ($row["betStatus"] == "NOT_ACCEPTED")
        {
            return "failed";
        }
        elseif ($row["betStatus"] == "PENDING_ACCEPTANCE")
        {
            return "pending";
        }
    }
