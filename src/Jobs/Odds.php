...
class Odds extends Job
{
    ...
    public function handle()
    {
        ...
        try {
            OutPut::normal('Start processing ODDS: ' . json_encode($this->message) . PHP_EOL);
            ...
            if (empty($request_uid) || empty($request_ts) || empty($provider) || empty($schedule) || empty($sport) || is_null($eventIds) || is_null($leagueIds)) {
                OutPut::normal("ODDS: Invalid payload." . PHP_EOL);
                $continue = false;
            }
    ...
    private function getService($request, $provider, $producer, $cache)
    {
        ...
        switch ($provider) {
            case 'pin':
                $service = new PinOdds($request, $producer, $cache);
                break;
        }
        return $service;
    }
