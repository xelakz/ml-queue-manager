...
use MultilineQM\Scraper\Provider\Pin\Leagues as PinLeagues;

class Leagues extends Job
{
    private $cache;
    private $producer;
    ...
    public function handle()
    {
        ...
        try {
            OutPut::normal('Start processing LEAGUES: ' . json_encode($this->message) . PHP_EOL);
            ...
            $continue = true;
            if (empty($request_uid) || empty($request_ts) || empty($provider) || empty($schedule) || empty($sport) || ($command != 'odd')) {
                OutPut::normal("LEAGUES: Invalid payload." . PHP_EOL);
        ...
        Helper::showExecutionTime('LEAGUES:', $start_time, json_encode($this->message));
    }
    ...
    private function getService($request, $provider, $producer, $cache)
    {
        ...
        switch ($provider) {
            case 'pin':
                $service = new PinLeagues($request, $producer, $cache);
                break;
        }
        return $service;
    }
