...
class Events extends Job
{
    ...
    public function handle()
    {
        ...
        try {
            OutPut::normal('Start processing EVENTS: ' . json_encode($this->message) . PHP_EOL);
            ...
            $continue = true;
            if (empty($request_uid) || empty($request_ts) || empty($provider) || empty($schedule) || is_null($leagueIds) || empty($sport)) {
                OutPut::normal("EVENTS: Invalid payload." . PHP_EOL);
        ...
        Helper::showExecutionTime('EVENTS:', $start_time, json_encode($this->message));
    }
    ...
    private function getService($request, $provider, $producer, $cache)
    {
        ...
        switch ($provider) {
            case 'pin':
                $service = new PinEvents($request, $producer, $cache);
                break;
        }
        return $service;
    }
