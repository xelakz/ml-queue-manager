...
use MultilineQM\Scraper\{Client\Request, QueueSession, Provider\Pin\Params as PinParams};
use MultilineQM\Scraper\Provider\Pin\RunningEvents as PinRunningEvents;

class RunningEvents extends Job
{
    private $cache;
    private $producer;
    ...
    public function handle()
    {
        ...
        try {
            OutPut::normal('Start processing RUNNING EVENTS: ' . json_encode($this->message) . PHP_EOL);
            ...
            $continue = true;
            if (empty($request_uid) || empty($request_ts) || empty($provider) || empty($sport) || is_null($leagueIds)) {
                OutPut::normal("RUNNING EVENTS: Invalid payload." . PHP_EOL);
                OutPut::normal(json_encode($this->message) . PHP_EOL);
                $continue = false;
            }
        ...
        Helper::showExecutionTime('RUNNING EVENTS:', $start_time, json_encode($this->message));
    }
    ...
    private function getService($request, $provider, $producer, $cache)
    {
        ...
        switch ($provider) {
            case 'pin':
                $service = new PinRunningEvents($request, $producer, $cache);
                break;
        }
        return $service;
    }
